...
def process(input, mask, anchors):
    ...
    #1차원부터 n차원의 4열 값을 box_confidence ... 은 numpy 에서만 
    box_confidence = input[..., 4]
    #axis = - 1 : 차원 추가 
    box_confidence = np.expand_dims(box_confidence, axis=-1)
    #1차원부터 n차원의 5열 값부터 box_ class_prob
    box_class_probs = input[..., 5:]

    box_xy = input[..., :2] *2 - 0.5
    #0~80 을 80번 반복해서 80열로 reshape
...
def yolov5_post_process(input_data):
    ...
    # ##############################
    # boxes[:,0]=boxes[:,0]*2
    # boxes[:,1]=boxes[:,1]*1.125
    # boxes[:,2]=boxes[:,2]*2
    # boxes[:,3]=boxes[:,3]*1.125
    # ###############################
